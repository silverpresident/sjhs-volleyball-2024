...
(function () {
    ...
    // State Management - Exposed globally for chat-members.js
    window.chatState = {
        connection: null,
        currentRoomId: null,
    ...
    function initializeSignalR() {
        ...
        // Expose connection globally
        window.chatConnection = state.connection;
    ...
    function registerHubMethods() {
        ...
        state.connection.on('UserAddedToRoom', (roomId, userId) => {
            console.log(`User ${userId} added to room ${roomId}`);
            // chat-members.js will handle the UI update
        });

        state.connection.on('UserRemovedFromRoom', (roomId, userId) => {
            console.log(`User ${userId} removed from room ${roomId}`);
            // chat-members.js will handle the UI update
        });

        state.connection.on('AddedToRoom', (roomInfo) => {
            console.log('You were added to room:', roomInfo);
            addRoomToSidebar(roomInfo);
            showNotification(`You were added to ${roomInfo.name}`, 'info');
        });

        state.connection.on('RemovedFromRoom', (roomId, roomName) => {
            console.log(`You were removed from room: ${roomName}`);
...
document.getElementById('message-input-area').style.display = 'none';
            ...
            showNotification(`You were removed from ${roomName}`, 'warning');
        });

        state.connection.on('RoomMembers', (roomId, memberIds) => {
            // This is handled by chat-members.js displayRoomMembers function
            if (typeof window.displayRoomMembers === 'function') {
                window.displayRoomMembers(memberIds);
            }
        });
    ...
    function joinRoom(roomId, roomName, roomType) {
        ...
        // Show manage members button for private rooms and direct chats
